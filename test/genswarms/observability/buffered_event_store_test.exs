defmodule Genswarms.Observability.BufferedEventStoreTest do
  ...
  describe "Buffered.Writer" do
    defp configure_writer(opts) do
      prev = Application.get_env(:genswarms, Buffered)

      Application.put_env(
        :genswarms,
        Buffered,
        Keyword.merge([inner: Genswarms.Observability.EventStore.Sqlite], opts)
      )

      ExUnit.Callbacks.on_exit(fn -> Application.put_env(:genswarms, Buffered, prev) end)
