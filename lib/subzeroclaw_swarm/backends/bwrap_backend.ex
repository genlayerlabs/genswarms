defmodule SubzeroclawSwarm.Backends.BwrapBackend do
  @moduledoc """
  ...
  @behaviour SubzeroclawSwarm.Backends.BackendBehaviour

  alias SubzeroclawSwarm.Backends.Bwrap.{OverlayManager, CgroupManager, AgentTelemetry}
  ...
  def start(name, config) do
    ...
    workspace = Map.get(config, :workspace, "/tmp/szc-workspace/#{sandbox_id}")
    |> Path.expand()
    ...
    case OverlayManager.setup_overlay(sandbox_id, presets) do
      {:ok, overlay_dir} ->
        ...
        try do
          ...
          e ->
            ...
            LogStore.log(:error, :backend, :bwrap_start_failed, "Failed to start bwrap: #{inspect(e)}",
              swarm: swarm_name,
              agent: String.to_atom(name),
      ...
      {:error, reason} ->
        LogStore.log(:error, :backend, :overlay_setup_failed, "Failed to setup overlay: #{inspect(reason)}",
          swarm: swarm_name,
          agent: String.to_atom(name),
  ...
  defp build_bwrap_command(sandbox_id, overlay_dir, skills_dir, workspace, _presets, config) do
    ...
    rest_args = [
      ...
      "--setenv", "SUBZEROCLAW_AGENT_NAME", to_string(name),
  ...
  defp find_subzeroclaw_binary(config) do
    ...
    explicit_path = Map.get(config, :subzeroclaw_path) ||
      Application.get_env(:subzeroclaw_swarm, :subzeroclaw_path)

    if explicit_path && File.exists?(explicit_path) do
      explicit_path
    else
      # Search common locations — check both CWD siblings and source repo siblings
      swarm_src = Path.expand("../..", :code.priv_dir(:subzeroclaw_swarm) |> to_string())
      search_paths = [
        # Sibling to CWD (when running from the swarm repo directly)
        Path.expand("../subzeroclaw/subzeroclaw", File.cwd!()),
        # Sibling to subzero-swarm source (when running from a dependency)
  ...
  defp find_wrapper_path do
    ...
    # even when subzeroclaw_swarm is a transitive dependency
    search_paths = [
      Path.join(:code.priv_dir(:subzeroclaw_swarm), "szc-wrapper-fifo.sh"),
      Path.join(:code.priv_dir(:subzeroclaw_swarm), "szc-wrapper.sh")
    ]
