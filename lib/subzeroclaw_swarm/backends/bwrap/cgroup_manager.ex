defmodule SubzeroclawSwarm.Backends.Bwrap.CgroupManager do
  @moduledoc """
  ...
  def kill_scope(scope_name) do
    ...
    System.cmd("systemctl", ["--user", "stop", "#{scope_name}.service"],
      stderr_to_stdout: true
    )

    # Also try scope for backwards compatibility
    System.cmd("systemctl", ["--user", "stop", "#{scope_name}.scope"],
      stderr_to_stdout: true
    )
  ...
  def list_active_scopes do
    case System.cmd("systemctl", ["--user", "list-units", "--type=scope", "--state=running", "--plain", "--no-legend"],
           stderr_to_stdout: true
         ) do
      {output, 0} ->
        output
