defmodule SubzeroclawSwarm.Observability.LogStore do
  @moduledoc """
  ...
  def subscribe do
    Phoenix.PubSub.subscribe(SubzeroclawSwarm.PubSub, @pubsub_topic)
  end
  ...
  def subscribe(swarm_name) do
    Phoenix.PubSub.subscribe(SubzeroclawSwarm.PubSub, "#{@pubsub_topic}:#{swarm_name}")
  end
  ...
  @spec unsubscribe() :: :ok
  def unsubscribe do
    Phoenix.PubSub.unsubscribe(SubzeroclawSwarm.PubSub, @pubsub_topic)
  end
  ...
  defp persist_to_sqlite(level, category, event_type, message, swarm, agent, metadata) do
    try do
      SubzeroclawSwarm.CLI.SwarmRegistry.log_event(
        level,
        category,
        event_type,
        message,
