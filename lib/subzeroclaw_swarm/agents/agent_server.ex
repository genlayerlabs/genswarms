defmodule SubzeroclawSwarm.Agents.AgentServer do
  @moduledoc """
  ...
  alias SubzeroclawSwarm.Agents.{AgentProtocol, Inbox}
  alias SubzeroclawSwarm.Observability.LogStore
  alias SubzeroclawSwarm.Config.SwarmConfig
  ...
  def via_tuple(swarm_name, agent_name) do
    {:via, Registry, {SubzeroclawSwarm.AgentRegistry, {swarm_name, agent_name}, :agent}}
  end
  ...
  def handle_info(:start_backend, state) do
    ...
    case state.backend_module.start(to_string(state.name), config) do
      {:ok, ref} ->
        ...
        {:ok, watcher} =
          SubzeroclawSwarm.Agents.LogWatcher.start_link(
  ...
  defp prepare_skills(state) do
    skills_base =
      Application.get_env(:subzeroclaw_swarm, :swarm_data_dir, "~/.subzeroclaw/swarms")
    ...
    priv_skills = Application.get_env(:subzeroclaw_swarm, :skills_dir, "priv/skills")
    project_root = Application.get_env(:subzeroclaw_swarm, :project_root) || File.cwd!()
