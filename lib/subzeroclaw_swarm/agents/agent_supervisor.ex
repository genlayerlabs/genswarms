defmodule SubzeroclawSwarm.Agents.AgentSupervisor do
  @moduledoc """
  DynamicSupervisor for agent processes.

  Provides functions to start, stop, and list agents within a swarm.
  Uses the application's DynamicSupervisor (SubzeroclawSwarm.AgentSupervisor)
  ...
  @supervisor SubzeroclawSwarm.AgentSupervisor
  ...
  def list_agents(swarm_name) do
    Registry.select(SubzeroclawSwarm.AgentRegistry, [
  ...
  defp find_agent_pid(swarm_name, agent_name) do
    case Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm_name, agent_name}) do
      [{pid, _}] -> {:ok, pid}
      [] -> :error
    end
  end
